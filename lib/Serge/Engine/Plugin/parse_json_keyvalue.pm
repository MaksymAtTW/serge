...
The following parsing errors were found when attempting to localize JSON key-value file.
</p>
...
sub parse {
    ...
    my $messages = from_json($$textref);

    if (!$messages) {
        my $error = "from_json() returned empty data structure";
        $self->{errors}->{$self->{parent}->{engine}->{current_file_rel}} = $error;
    ...
    foreach my $key (sort keys %$messages) {
        my $msg = $messages->{$key};
        $messages->{$key} = &$callbackref($msg, undef, undef, undef, $lang, $key);
    }
